...
type Entity = Car | User;

async function query(entityType: string, delay = 10): Promise<any[]> {
...
async function post(entityType: string, newEntity: Entity): Promise<any> {
...
async function put(entityType: string, updatedEntity: Entity): Promise<any> {
    const entities = await query(entityType);
...
async function remove(entityType: string, entityId: string): Promise<any> {
    const entities = await query(entityType);
